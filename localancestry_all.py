...
def batchList(samples_length, num_batches):
	step_size = samples_length // num_batches
	first=[0]
	last=[0]
...
def beneTract(merged_tracts, locus):
	'''
	Returns tract length, start, and end for tract spanning bene_locus
	'''
...
#def childLoop(samples, anc, locus, outfile):
	'''
	Loops through samples to calculate
	bene_tract lengths, ancestry switches,
	and global ancestry. Population used for
	global ancestry will be based on the 
	ancestry at bene_locus. Writes to outfile.
	'''
#	if anc == "P1":
#		anc_map = dumpTracts(samples, 0, 4)
#	if anc == "P2":
#		anc_map = dumpTracts(samples, 4, 8)
...
#		outfile.write(f"{tract_length}\t{tract_start}\t{tract_end}\t{anc}\t{switches}\t{global_ancestry}\t{child}\n")
...
#		flush_count = 0
		...
		for anc in ["P1", "P2"]:
			...
			batch_first, batch_last = batchList(len(samples), batch_num)
...
#					flush_count +=1
	...
	#	childLoop(bene_samples, "P1", bene_locus, out)
	#	childLoop(non_bene_samples, "P2", bene_locus, out)
